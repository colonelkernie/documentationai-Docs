...
description: >-
  Easily keep your Assets availability and driver status up to date in Copilot
  with a Chrome extension.
---
## Sync Assets and Drivers Data from Amazon Relay

Asset availability and driver status are constantly changing, and it can be difficult for AFP owners and managers to constantly update Laminar Copilot with the latest status of each asset and driver in your fleet.

To automate the update process, Laminar Copilot provides a Chrome extension that will automatically sync your Assets and Drivers data from Amazon Relay to Laminar Copilot.
...
2. Navigate to Amazon Relay and open the page for the data you want to sync. (If you are already on that page, refresh the page.)

   - **Assets:** the [Assets page](https://relay.amazon.com/assets?ref=owp_nav_assets)

   - **Drivers:** the [Drivers page](https://relay.amazon.com/drivers)
...
4. After the "debug" session, navigate to the matching page in Laminar Copilot and confirm it has been updated with the latest data from Amazon Relay.

   - **Assets:** the [Assets page of Laminar Copilot](https://app.laminarcopilot.com/tractors)

   - **Drivers:** the [Drivers page of Laminar Copilot](https://app.laminarcopilot.com/drivers)
...
- The SCV Extension will sync the **driver status** of drivers in Amazon Relay with drivers in Laminar Copilot, matched by phone number. A driver's phone number must be the same in both platforms for the sync to match them.

- If there are drivers in Amazon Relay that are **not in Laminar Copilot**, the extension will not add those drivers to Copilot.

- If there are drivers in Laminar Copilot that are **not in Amazon Relay**, the extension will not delete those drivers from Copilot.

- Drivers must first be created and deleted in Laminar Copilot; the extension will only update drivers that exist in both platforms.
