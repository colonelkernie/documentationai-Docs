...
3. **Select the Route Type** — choose the appropriate route type (Solo 1, Solo 2, Solo 5, RLB, or INC). The fields required to complete scheduling will vary based on the route type (see below).

4. **Enter required details** — fill in all mandatory fields for the route type you selected. For example, Solo 1, Solo 2, and Solo 5 routes require Block ID, Start Date, Contract, Start Point, End Point, Driver, and Asset.
...
## Solo 1, Solo 2, or Solo 5 Routes

If a route has the route type of **Solo 1**, **Solo 2**, or **Solo 5**, the following fields are required:
...
For Solo 1, Solo 2, and Solo 5 routes, Copilot will automatically calculate the route's ending date and time based on the route type. Solo 1 routes have a set length of 14 hours. Solo 2 routes have a set length of 38 hours. Solo 5 routes have a set length of 70 hours.
...
The dropdown also shows how many hours each driver has already worked that week, so you can see who has room before you assign. Within each priority group, drivers are listed from fewest hours worked to most.
...
- The asset is not assigned to another route during the duration of the route being scheduled.
